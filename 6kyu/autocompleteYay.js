...
function autocomplete(str, arr){
    let newStr = str.toLowerCase().split('').filter(y => 'abcdefghijklmnopqrstuvwxyz'.includes(y)).join('');
    return arr
    .filter(x => x.toLowerCase().split('').filter(y => 'abcdefghijklmnopqrstuvwxyz'.includes(y)).join('').substring(0,newStr.length) 
    == str.toLowerCase().split('').filter(y => 'abcdefghijklmnopqrstuvwxyz'.includes(y)).join('').substring(0,str.length))
    .slice(0,5);
}
