...
// MY SOLUTION
function fizzbuzzBackwards(arr){
    let fizzBuzz = (arr.indexOf('FizzBuzz') + 1);
    let fizz = (arr.indexOf('Fizz') > -1) ? (arr.indexOf('Fizz') + 1) : null;
    let buzz = (arr.indexOf('Buzz') > -1) ? (arr.indexOf('Buzz') + 1) : null;
    if(fizz && buzz){
        return [fizz, buzz];
    }else if(!fizz && !buzz){
