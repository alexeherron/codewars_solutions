...
function travel(r, zip){
    let addressList = r.split(',').filter(x => x.slice(-8) == zip).map(x => x.replace(zip, '').trim());
    let addresses = addressList.map(x => x.split(' ').slice(1).join(' '));
    if(addressList.length < 1){
        addresses = '/';
    }else{
        addresses = addresses + `/${addressList.map(x => x.split(' ')[0]).join(',')}`
    }
    return `${zip}:${addresses}`
}
