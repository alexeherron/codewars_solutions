...
//MY SOLUTION
function fixPhoneNumbers(string){
    let mashingTheKeyboard = string.split('').filter(x => (/[0123456789]/g).test(x) ? x : '');
    return (mashingTheKeyboard.length == 11 && mashingTheKeyboard[0] == 0) ? mashingTheKeyboard.join('') : 'Not a phone number';
}
