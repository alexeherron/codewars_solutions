...
//MY SOLUTION
function checkDigit(n, in1, in2, dig){
    if(in2 > in1){
        return String(n).split('').slice(in1, (in2 + 1)).includes(String(dig));
    }else if(in1 > in2){
        return String(n).split('').slice(in2, (in1 + 1)).includes(String(dig));
    }else{
        return String(n)[in1] == dig;
    }
}
